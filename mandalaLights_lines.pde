...
if(mousePressed){
    ...
    vertex( cy, cy, z);
    endShape();
    harom(ax*ratio+cx*(1-ratio), ay*ratio+cy*(1-ratio), ax*(1-ratio)+bx*ratio, ay*(1-ratio)+by*ratio, level-1, ratio, nx+8);
  }
}}
void draw()
{//scale(1.02);
  ...
  stroke(off%216*0.8, 196, 224,88);  
  //noStroke();
  fill(off%216*8.8,16, 224,288);
  off+=0.8;

  harom(500, 460, 45, 460, 10, (sin(0.00088*millis()%(16*TWO_PI))+1)/8, -8);
}
//}